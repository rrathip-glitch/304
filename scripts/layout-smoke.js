...
assert(
  /\.seat-cards\.vertical[\s\S]*?height:\s*252px[\s\S]*?max-height:\s*100%/.test(css),
  'side opponent vertical stacks are statically sized (v2.2.24: 252 px, caps at cell)',
);
assert(
...
assert(/v=2\.2\.27/.test(html), 'index.html uses semver cache-bust marker (v2.2.27)');
...
assert(
  /BUILD\s*=\s*'2\.2\.27'/.test(client),
  'client.js declares BUILD = "2.2.27" (semver, not codename)',
);
assert(
